...
class KeyPresserApp:
    ...
    def send_key(self, hwnd, key, delay=0.2):
        """Send a key press and release to a specific window."""
        try:
            vk_code = ord(key.upper())  # Convert key to virtual key code
            scan_code = win32api.MapVirtualKey(vk_code, 0)

            # Send WM_KEYDOWN
            for _ in range(1):
                win32api.PostMessage(hwnd, win32con.WM_KEYDOWN, vk_code, scan_code << 16)
                time.sleep(delay)
                # Send WM_KEYUP
                win32api.PostMessage(hwnd, win32con.WM_KEYUP, vk_code, scan_code << 16)
            print(f"Key '{key}' sent to window {hwnd}")
        except Exception as e:
            print(f"Error sending key '{key}' to window {hwnd}: {e}")
